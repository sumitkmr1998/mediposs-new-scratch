...
class SubscriptionService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    final lastTimeMs = prefs.getInt('lastKnownTime') ?? 0;
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    if (nowMs < lastTimeMs) {
      debugPrint('SubscriptionService: Clock tampering detected on init! System clock was rolled back. Reverting to Free locally.');
      _currentTier = UserTier.free;
      _licenseKey = '';
      _licenseExpiry = null;
    } else {
      await prefs.setInt('lastKnownTime', nowMs);
    }

    // Set up continuous background clock tampering check
    _clockTamperTimer?.cancel();
    _clockTamperTimer = Timer.periodic(const Duration(seconds: 10), (timer) async {
      final p = await SharedPreferences.getInstance();
      final lastMs = p.getInt('lastKnownTime') ?? 0;
      final currentMs = DateTime.now().millisecondsSinceEpoch;
      if (currentMs < lastMs) {
        debugPrint('SubscriptionService: Background clock tampering detected! Reverting to Free.');
        _currentTier = UserTier.free;
        _licenseKey = '';
        _licenseExpiry = null;
        notifyListeners();
      } else {
        await p.setInt('lastKnownTime', currentMs);
      }
    });
  ...
  Future<void> _verifyLicenseWithCloud() async {
    ...
    try {
      ...
      final robustNow = await TimeService.getRobustTime();
      if (_licenseExpiry != null && _licenseExpiry!.isBefore(robustNow)) {
  ...
  Future<void> _updateLocalLicense(String key, DateTime? expiry, String tier) async {
    ...
    // Save the latest known time whenever we update local license status
    final nowMs = DateTime.now().millisecondsSinceEpoch;
    final lastTimeMs = prefs.getInt('lastKnownTime') ?? 0;
    if (nowMs >= lastTimeMs) {
      await prefs.setInt('lastKnownTime', nowMs);
